...
# ==============================================================================
# CONFIG DATABASE AIVEN MYSQL (ĐÃ LÀM SẠCH KÝ TỰ ẨN)
# ==============================================================================
DB_USER = "avnadmin"
...
def init_db():
    """Tự tạo bảng orders nếu chưa có"""
...
# Khởi tạo bảng ngay khi bật app
try:
    init_db()
except Exception as e:
    st.error(f"Lỗi kết nối MySQL Aiven: {e}")

# Thực đơn nhà hàng Mr. Bình
menu = {
    "Đồ ăn": {
...
def load_history_from_db():
    """Lấy dữ liệu từ MySQL về DataFrame"""
    try:
        engine = get_db_engine()
...
if page == "🍽️ Order":
            ...
            with col_btn1:
                if st.button("💳 Thanh toán"):
                    ...
                    try:
                        ...
                        st.success(
                            "Thanh toán thành công! Dữ liệu đã lưu lên Aiven MySQL."
                        )
                        st.session_state.order_dict = {}
                        st.rerun()
                    except Exception as e:
    ...
    with tab2:
        ...
        if not df_history.empty:
            ...
            st.info("Hệ thống chưa ghi nhận bất kỳ giao dịch thanh toán nào.")
    ...
    with tab3:
        ...
        if not df_anal.empty:
            ...
            with col_kpi3:
                ...
                st.metric(
                    label=f"Tháng {best_month}", value=f"{best_month_rev:,.0f} VNĐ"
                )
            ...
            with col_info2:
                st.markdown(
                    f"👉 Khung giờ đắt khách nhất: **{best_hour:02d}:00 - {(best_hour+1):02d}:00** (**{best_hour_qty} phần**)."
                )
                st.dataframe(
